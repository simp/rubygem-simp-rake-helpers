...
#FIXME this test fails for pre-SIMP6
  ...
  it 'should not require pupmod-simp-simplib for simp-simplib RPM' do
    ...
    simplib_rpm = File.join(pkg_root_dir, 'simplib', 'dist', File.basename(testpackage_rpm))
...
shared_examples_for "a pre-SIMP6 RPM generator" do
  it "should create an RPM and leave the mock directory" do
    test_name %(runs SIMP_RAKE_MOCK_cleanup=no #{env_vars[build_type]} rake pkg:rpm[epel-#{test_dist}-x86_64,true])
    ...
    test_name 'RPM generated does not require simp-adapter'
    ...
    test_name 'produces RPM without SIMP6-specific appropriate pre/post/preun/postun'
    on test_host,
      ...
      dists.each do |dist|
        context "for #{dist} distribution using simpdefault.spec" do
          ...
          it "should create an RPM for #{dist} and leave the mock directory" do
            test_name %(runs SIMP_RAKE_MOCK_cleanup=no rake pkg:rpm[epel-#{dist}-x86_64,true])
