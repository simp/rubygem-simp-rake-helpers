      ...
      dists.each do |dist|
        context "on #{dist}" do
          it "should create an RPM for #{dist} and leave the mock directory" do
            ...
            on host, %(rpm -qp --scripts #{testpackage_rpm} | grep -q -x '/usr/local/sbin/simp_rpm_helper --rpm_dir=/usr/local/share/simp/modules/testpackage --rpm_section=pre --rpm_status=$1')
            on host, %(rpm -qp --scripts #{testpackage_rpm} | grep -q -x '/usr/local/sbin/simp_rpm_helper --rpm_dir=/usr/local/share/simp/modules/testpackage --rpm_section=post --rpm_status=$1')
            on host, %(rpm -qp --scripts #{testpackage_rpm} | grep -q -x '/usr/local/sbin/simp_rpm_helper --rpm_dir=/usr/local/share/simp/modules/testpackage --rpm_section=preun --rpm_status=$1')
            on host, %(rpm -qp --scripts #{testpackage_rpm} | grep -q -x '/usr/local/sbin/simp_rpm_helper --rpm_dir=/usr/local/share/simp/modules/testpackage --rpm_section=postun --rpm_status=$1')
