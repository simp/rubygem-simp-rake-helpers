...
describe Simp::Build::Mock do
  ...
  describe '#buildrpm' do
    ...
    it "builds an RPM" do
      pkg_dir = File.join(test_files_path, 'buildrpm_files', 'build')
      srpm_file = File.join(test_files_path, 'buildrpm_files', 'testpackage-1-0.src.rpm')
      rpm_file = File.join(resultdir_path, 'testpackage-1-0.noarch.rpm' )
      mm.run("mkdir -p #{resultdir_path}")
      mm.copyin(srpm_file,pkg_dir,resultdir_path)
      mm.run("chmod -R ugo+rwX #{resultdir_path}")

      expect( mm.buildrpm(srpm_file).success? ).to be true
      expect( File.file?(rpm_file) ).to be true
    end
  end
  ...
  after :each do
    FileUtils.rm_rf Dir[File.join(@resultdir_path,'*')]
#    if m.run("test -d #{resultdir_path}/build").success?
#      m.run("rm -rf  #{resultdir_path}/* ")
#    end
  end
