...
class Simp::Ci::Gitlab
  ...
  # a valid nodeset
  #
  # This task will fail if an acceptance test job is missing the
  # suite or nodeset or contains and invalid suite or nodeset.
  #
  # +component_dir+:: The root directory of the component project.
  def self.validate_acceptance_tests(component_dir)
    ...
    gitlab_yaml.each do |key,value|
      ...
      value['script'].each do |line|
        ...
        if line.include?('[')
          ...
          unless Dir.exist?(File.join(component_dir, 'spec', 'acceptance',
            'suites', suite)
