...
class Simp::RelChecks
  ...
  def self.compare_latest_tag(component_dir, tags_source = 'origin', verbose = false)
    info, = load_and_validate_changelog(component_dir, verbose)
      ...
      if tags.empty?
        ...
        if files_changed.empty?
          ...
          if curr_version < last_tag_version
            raise("ERROR: Version regression. '#{info.version}' < last tag '#{last_tag}'")
  ...
  def self.create_tag_changelog(component_dir, verbose = false)
    ...
    changelogs.each do |entry|
      result += "\n#{entry[:content].first}\n"
      next unless entry[:content].size > 1

      entry[:content][1..].each do |line|
        result += "  #{line}\n"
      end
    end
    result
  ...
  def self.extract_version_changelog(changelog_entries, version,
                                     release = nil, _verbose = false)
    changelogs = []
    changelog_entries.each do |entry|
      if entry[:version] > version
        raise("ERROR: Changelog entry for version > #{version} found: \n #{entry[:content].join("\n")}")
        ...
        if release && entry[:release] && release.match(%r{^#{entry[:release]}}).nil?
          raise("ERROR: Version release does not match #{release}: \n #{entry[:content].join("\n")}")
        end
    ...
    if changelogs.empty?
      raise("ERROR: No valid changelog entry for version #{version} found")
    end
