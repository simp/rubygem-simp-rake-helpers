...
module Simp
  module Simp::Build
    ...
    class Simp::Build::Mock
      ...
      # Criticality levels
      #
      # * `:verbose` = more output from mock
      # * `:normal`  = normal output from mock
      # * `:quiet`   = skip output
      # * `:silent`  = skip command && output
      VERBOSE_LEVELS = [:silent,:quiet,:normal,:verbose]

      # @!attribute [r]
      #   Verbosity level for this mock instance
      ...
      def mock_cmd
        ...
        cmd = cmd + ['--no-cleanup-after'] unless @cleanup_after
        cmd = cmd + [
                      '--no-clean',
      ...
      # @parameter *arg arguments(s) to pass to `mock`
      def run(*arg)
        raise NotImplementedError, 'TODO run(*arg)!'
      end
